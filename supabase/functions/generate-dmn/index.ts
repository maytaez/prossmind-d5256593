...
function ensureDmndiExists(xml: string): string {
  // Always strip existing DMNDI section to ensure we generate a valid one with correct edge references
  // The LLM often generates incorrect DMNDI (e.g. edges pointing to decisions instead of requirements)
  if (xml.includes('<dmndi:DMNDI>')) {
    console.log('[DMNDI Fix] Removing existing DMNDI section to regenerate it...');
    xml = xml.replace(/<dmndi:DMNDI>[\s\S]*?<\/dmndi:DMNDI>/gi, '');
  }
  if (xml.includes('<dmndi:DMNDiagram')) {
    // Fallback for cases where DMNDI tag might be different or partial
    xml = xml.replace(/<dmndi:DMNDiagram[\s\S]*?<\/dmndi:DMNDiagram>/gi, '');
  }

  console.log('[DMNDI Fix] Generating fresh DMNDI section...');
  ...
  // Calculate total heights to center columns vertically
  const inputRowHeight = 100;
  const decisionRowHeight = 150;
  const totalInputHeight = inputData.length * inputRowHeight;
  const totalDecisionHeight = decisions.length * decisionRowHeight;
  const maxHeight = Math.max(totalInputHeight, totalDecisionHeight);

  const inputStartY = 100 + Math.max(0, (maxHeight - totalInputHeight) / 2);
  const decisionStartY = 100 + Math.max(0, (maxHeight - totalDecisionHeight) / 2);

  // Add shapes for input data (positioned on the left)
  let inputY = inputStartY;
  ...
  for (const req of infoReqs) {
    ...
    if (isInput) {
      sourceYPos = inputStartY + (sourceIndex * inputRowHeight) + 22.5; // Center of input shape
      sourceXPos = 275; // Right edge of input (150 + 125)
    } else {
      sourceIndex = decisions.findIndex(dec => dec.id === req.sourceId);
      if (sourceIndex !== -1) {
        sourceYPos = decisionStartY + (sourceIndex * decisionRowHeight) + 40; // Center of decision shape
        sourceXPos = 680; // Right edge of decision (500 + 180)
    ...
    if (sourceIndex !== -1 && decisionIndex !== -1) {
      const targetYPos = decisionStartY + (decisionIndex * decisionRowHeight) + 40; // Center of target decision
...
function fixDmndiLayout(xml: string): string {
  ...
  fixed = fixed.replace(/<dmndi:DMNShape[^>]*dmnElementRef="([^"]+)"[^>]*>[\s\S]*?<dc:Bounds\s+x="([^"]+)"\s+y="([^"]+)"\s+width="([^"]+)"\s+height="([^"]+)"\s*\/>/gi, (match, elementRef, x, y, width, height) => {
    ...
    let widthNum = parseFloat(width) || (isDecision ? 180 : 125);
    let heightNum = parseFloat(height) || (isDecision ? 80 : 45);

    // Ensure minimum and maximum reasonable sizes
    if (isDecision) {
      // Decision tables need larger bounds
      if (widthNum < 160) widthNum = 180;
      if (widthNum > 800) widthNum = 600; // Cap at reasonable max
      if (heightNum < 60) heightNum = 80;
      if (heightNum > 600) heightNum = 400; // Cap at reasonable max
    } else if (isInputData) {
      // Input data can be smaller
      if (widthNum < 80) widthNum = 125;
      if (widthNum > 300) widthNum = 200;
      if (heightNum < 30) heightNum = 45;
