...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Proceed even if user is null (Guest mode)
    ...
    console.log(`Processing ${diagramType.toUpperCase()} diagram for ${user ? "User " + user.id : "Guest"}`);
    // Create job record immediately
    // IMPORTANT: 'user_id' must be nullable in the database for guest uploads to work
    const { data: job, error: jobError } = await supabase
      .from("vision_bpmn_jobs")
      .insert({
        user_id: user ? user.id : null, // Set to null if guest
    ...
    if (jobError || !job) {
      ...
      throw new Error("Failed to create processing job - ensure vision_bpmn_jobs.user_id is nullable");
    }
    ...
    const processJob = async () => {
      ...
      try {
        ...
        await supabase.from("vision_bpmn_jobs").update({ status: "processing" }).eq("id", job.id);
        console.log("Processing document for BPMN generation for user:", user ? user.id : "Guest");
        ...
        const isDocx = imageBase64.startsWith(
        ...
        if (isDocx || isDoc) {
          throw new Error(
            "Word documents (.docx, .doc) are not supported by Gemini. Please convert your document to a PDF or extract the text and paste it directly.",
          );
        }
        ...
        if (isImage || isPDF) {
          ...
          if (visionCache) {
            console.log(
              "✅ EXACT VISION CACHE HIT - Found cached data for image hash:",
            ...
            console.log(
              "❌ Exact vision cache miss - No cached data found for hash:",
              imageHash.substring(0, 16) + "...",
            );
            ...
            if (isSemanticCacheEnabled() && isImage && LOVABLE_API_KEY) {
              ...
              try {
                ...
                if (quickDescResponse.ok) {
                  ...
                  if (imageDescription) {
                    console.log(
                      "Generated image description for embedding:",
                      imageDescription.substring(0, 100) + "...",
                    );
                    // Generate embedding from description
                    const embedding = await generateEmbedding(imageDescription);
                    // Check semantic cache
                    const semanticCache = await checkSemanticImageCache(embedding, diagramType, 0.8);
                    if (semanticCache && semanticCache.bpmnXml) {
                      console.log(
                        "✅ SEMANTIC VISION CACHE HIT - Found similar image! Similarity:",
                        semanticCache.similarity,
                      );
...
Return ONLY the XML, no other text.`;
          ...
          throw new Error(
            "Unsupported file type. Please upload an image, PDF, Word document, or provide text content.",
          );
        }
        ...
        if (!bpmnXml.includes("<bpmn:definitions") && !bpmnXml.includes("<bpmn:Definitions")) {
          throw new Error(
            "Generation failed (Reason: failed to parse document as <bpmn:Definitions>). Try simplified prompt.",
          );
        }
