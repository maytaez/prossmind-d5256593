...
export function parseStructureXml(xml: string): {
  ...
  const elementPatterns = [
    ...
    {
      type: "boundaryEvent",
      regex: /<bpmn:boundaryEvent[^>]*id="([^"]*)"[^>]*name="([^"]*)"[^>]*attachedToRef="([^"]*)"[^>]*>/g,
    },
    {
      type: "intermediateThrowEvent",
      regex: /<bpmn:intermediateThrowEvent[^>]*id="([^"]*)"[^>]*name="([^"]*)"[^>]*>/g,
    },
  ];
  ...
  const flowMatches = xml.matchAll(
    /<bpmn:sequenceFlow[^>]*id="([^"]*)"[^>]*sourceRef="([^"]*)"[^>]*targetRef="([^"]*)"[^>]*(?:name="([^"]*)")?[^>]*>/g,
  );
