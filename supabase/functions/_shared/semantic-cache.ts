...
async function checkCacheInternal(options: CacheCheckOptions): Promise<CacheResult | null> {
    ...
    try {
        ...
        if (similarMatches && similarMatches.length > 0) {
            ...
            const duration = Date.now() - startTime;
            console.log(`[Cache] ✅ Semantic match found! Similarity: ${(match.similarity * 100).toFixed(1)}%, retrieved in ${duration}ms`);

            // Update hit count asynchronously
            updateCacheHitCount(match.id, supabase).catch(err =>
                console.warn('[Cache] Failed to update hit count:', err)
            );
