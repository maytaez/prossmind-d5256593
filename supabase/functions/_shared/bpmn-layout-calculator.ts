...
function calculateBounds(graph: Map<string, GraphNode>, startX: number, startY: number): Map<string, Bounds> {
  ...
  Array.from(levelGroups.keys())
    ...
    .forEach((level) => {
      ...
      const maxHeight = Math.max(...levelGroups.get(level)!.map((node) => getElementSize(node.element).height));
 ...
 */
function calculateWaypoints(flow: BPMNSequenceFlow, sourceBounds: Bounds, targetBounds: Bounds): Point[] {
...
function calculateSubprocessLayout(element: BPMNElement): Bounds {
  ...
  // Add padding
  const width = Math.max(subLayout.totalWidth + LAYOUT_CONFIG.subprocessPadding * 2, LAYOUT_CONFIG.minSubprocessWidth);
 ...
 */
export function constrainElementsToLanes(boundsMap: Map<string, Bounds>, lanes: LaneLayout[]): Map<string, Bounds> {
...
export function positionBoundaryEvents(
  ...
  groupedEvents.forEach((events, attachedToId) => {
    ...
    events.forEach((event, index) => {
      ...
      const bounds = calculateBoundaryEventPosition(event.id, attachedToId, attachedToBounds, "bottom", offset);
