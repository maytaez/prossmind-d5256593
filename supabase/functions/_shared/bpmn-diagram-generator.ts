...
export async function addBpmnDiagram(structureXml: string): Promise<string> {
    ...
    console.log(`[addBpmnDiagram] Found ${process.elements.length} elements, ${process.flows.length} flows, ${process.lanes.length} lanes`);
    ...
    if (process.lanes.length > 0) {
        ...
        totalWidth = maxX - 200;
        totalHeight = laneLayoutResult.laneLayouts[laneLayoutResult.laneLayouts.length - 1]?.y +
            laneLayoutResult.laneLayouts[laneLayoutResult.laneLayouts.length - 1]?.height - 100;
    } else {
    ...
    const boundaryEvents = process.elements.filter((el: any) =>
        el.type === 'boundaryEvent' && el.attachedToRef
    );
    ...
    const baseLayout = process.lanes.length === 0
        ? calculateLayout({ id: process.id, name: process.name, elements: process.elements, flows: process.flows } as any, 200, 100)
