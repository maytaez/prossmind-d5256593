...
async function getMetricsOverview(supabase: any) {
  ...
  const successfulRequests =
    totalStats?.filter((r: any) => r.status === "success" || r.status === "cached").length || 0;
...
async function getRequestDetail(supabase: any, logId: string) {
  const { data, error } = await supabase.from("bpmn_generation_logs").select("*").eq("id", logId).single();
...
async function getErrorSummary(supabase: any) {
  const { data, error } = await supabase.from("dashboard_error_summary").select("*").limit(50);
