...
-- Schedule daily cleanup via pg_cron (requires pg_cron extension)
-- Note: This will fail if pg_cron is not installed, but logs will still work
DO $$
BEGIN
  -- Check if pg_cron extension exists
  IF EXISTS (SELECT 1 FROM pg_extension WHERE extname = 'pg_cron') THEN
    -- Remove existing job if it exists
    PERFORM cron.unschedule('cleanup-dashboard-logs');
    
    -- Schedule new job to run daily at 2 AM
    PERFORM cron.schedule(
      'cleanup-dashboard-logs',
      '0 2 * * *', -- Daily at 2 AM
      $$SELECT cleanup_old_dashboard_logs(30);$$
    );
    
    RAISE NOTICE 'Scheduled daily cleanup job at 2 AM';
  ELSE
    RAISE NOTICE 'pg_cron extension not found, skipping scheduled cleanup. Run cleanup_old_dashboard_logs() manually.';
  END IF;
EXCEPTION
  WHEN OTHERS THEN
    RAISE NOTICE 'Could not schedule cleanup job: %. Run cleanup_old_dashboard_logs() manually.', SQLERRM;
END $$;
