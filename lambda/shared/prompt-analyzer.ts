...
function quickComplexityCheck(prompt: string): PromptAnalysis {
  ...
  const businessActors = (
    prompt.match(
      /customer|client|user|manager|underwriter|reviewer|approver|senior|admin|operator|agent|analyst|specialist|engineer|auditor|officer|lead|team/gi,
    ) || []
  ).length;
  ...
  const complexKeywords = (
    prompt.match(
      /swimlane|pool|lane|timer|boundary|escalate|recertification|parallel|subprocess|nested|compensation|error handling|exception|message flow|signal|conditional/gi,
    ) || []
  ).length;
  ...
  const timerKeywords = (
    prompt.match(
      /timeout|auto-cancel|auto-reject|auto-close|escalate|escalation|wait|delay|days?|hours?|minutes?|deadline|expire|within|after/gi,
    ) || []
  ).length;
  ...
  const supportWorkflowKeywords = (
    prompt.match(
      /ticket|support|queue|routing|categorize|category|priority|high priority|low priority|medium priority|request|portal|submission|workflow|notification|notify|message|alert|response/gi,
    ) || []
  ).length;
  ...
  const gatewayKeywords = (
    prompt.match(
      /exclusive gateway|gateway|route|based on|if\s|then\s|else\s|otherwise|depending on|categorize|assign to|send to|trigger/gi,
    ) || []
  ).length;

  // Multi-department/multi-actor indicators (NEW)
  const multiActorKeywords = (
    prompt.match(/different|multiple|various|several|team|teams|department|across|between/gi) || []
  ).length;

  const totalComplexityIndicators =
    complexKeywords +
    businessKeywords +
    timerKeywords +
    loopKeywords +
    supportWorkflowKeywords +
    gatewayKeywords +
    multiActorKeywords;
  ...
  if (supportWorkflowKeywords >= 3 && (totalActors >= 2 || timerKeywords >= 1 || gatewayKeywords >= 1)) {
    console.log(
      `[Quick Check] Support workflow detected: ${supportWorkflowKeywords} support keywords, ${totalActors} actors, ${timerKeywords} timers`,
    );
    return {
      isComplex: true,
      complexity: {
  ...
  if (totalActors >= 3 && (timerKeywords >= 1 || gatewayKeywords >= 2 || complexKeywords >= 2)) {
    console.log(
      `[Quick Check] Multi-actor complex workflow: ${totalActors} actors, ${timerKeywords} timers, ${gatewayKeywords} gateways`,
    );
    return {
      isComplex: true,
      complexity: {
...
export function fallbackAnalysis(prompt: string): PromptAnalysis {
  ...
  const businessActors = (
    prompt.match(
      /underwriter|reviewer|approver|senior|admin|operator|agent|analyst|specialist|engineer|auditor|officer|client|user|lead/gi,
    ) || []
  ).length;
  const totalActors = actors + businessActors;
  ...
  const gateways = (
    prompt.match(
      /gateway|parallel|exclusive|inclusive|decision|branch|if|else|otherwise|based on|where required|conditional/gi,
    ) || []
  ).length;
  const events = (
    prompt.match(
  ...
  const timerEvents = (
    prompt.match(
      /timeout|auto-cancel|auto-reject|auto-close|deadline|expire|wait|delay|days?|hours?|minutes?|within|after|escalation/gi,
    ) || []
  ).length;
  ...
  const complexFeatures = (
    prompt.match(
      /timer event|boundary event|message flow|compensation|error handling|escalation|approval|access management|provisioning|deprovisioning|recertification|automated|confirmation|verify|validate|countersign|authorize|ticket|support|queue|routing|categorize|priority|notification|notify/gi,
    ) || []
  ).length;
  ...
  if (
    estimatedXmlSize > 70000 || // Lowered from 80000
