...
const Navigation = ({ user: userProp }: { user?: User | null }) => {
  ...
  const docsUrl = import.meta.env.VITE_DOCS_URL || 'http://localhost:8082';
  
  const navItems = [
    { name: "Features", path: "/features" },
    { name: "Vision AI", path: "/vision-ai" },
    { name: "Pricing", path: "/pricing" },
    { name: "Documentation", path: docsUrl, external: true },
  ...
  return (
    <motion.nav 
      ...
      initial={false}
      animate={{
        backgroundColor: isScrolled ? "hsl(var(--background) / 0.98)" : "hsl(var(--background) / 0.95)",
                    ...
                    if (item.external) {
                      return (
                        <a
                          key={item.path}
                          href={item.path}
                          className="relative px-5 py-2 rounded-full text-sm font-medium transition-all duration-300 whitespace-nowrap text-foreground/70 hover:text-foreground hover:bg-muted/80 hover:scale-[1.02] active:scale-[0.98]"
              ...
              {user ? (
                <div className="flex items-center gap-3 flex-shrink-0">
                  <Button
                    ...
                    className="gap-2 hover:bg-primary/10 hover:text-primary transition-all whitespace-nowrap hidden lg:flex"
                ...
                <div className="flex items-center gap-3 flex-shrink-0">
                  ...
                  <ThemeToggle />
                  <a href={`${appUrl}/auth`} className="flex-shrink-0">
                    <Button className="flex items-center gap-2 bg-primary hover:bg-primary/90 shadow-md hover:shadow-lg hover:scale-[1.02] active:scale-[0.98] transition-all whitespace-nowrap" aria-label="Sign up or log in">
                      Sign In
              ...
              <SheetContent side="right" className="w-[300px] sm:w-[400px]">
                ...
                <nav className="flex flex-col gap-4 mt-6" role="navigation" aria-label="Main navigation">
                  ...
                  {navItems.map((item, index) => {
                    if (item.external) {
                      return (
                        <a
                          ...
                          className="px-4 py-2 rounded-lg text-sm font-medium transition-all duration-normal focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring text-foreground/70 hover:text-foreground hover:bg-muted/80"
                          aria-label={`Navigate to ${item.name} page`}
                          style={{
                    ...
                    return (
                      <Link
                        ...
                        to={item.path}
                        onClick={() => setIsSheetOpen(false)}
                        className={`px-4 py-2 rounded-lg text-sm font-medium transition-all duration-normal focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-ring ${
                          location.pathname === item.path
                  ...
                  {user ? (
                    ...
                    <a href={`${appUrl}/auth`} onClick={() => setIsSheetOpen(false)}>
